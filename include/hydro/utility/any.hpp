...
public:
	...
	template <typename T>
	any(const T x) : mContent{new holder<typename std::decay<const T>::type>(x)} {}
	...
	any &operator=(T &&x)
	{
		mContent.reset(new holder<typename std::decay<T>::type>(typename std::decay<T>::type(x)));
		return (*this);
	}
...
T any_cast(any &x)
{
	if (x.mContent->type() != typeid(T))
		throw bad_any_cast();

	return static_cast<any::holder<T> *>(x.mContent.get())->value;
}
...
T *any_cast(any *x)
{
	return dynamic_cast<T *>(x->mContent.get());
}

template <typename T>
const T *any_cast(const any *x)
{
	return dynamic_cast<const T *>(x->mContent.get());
}
