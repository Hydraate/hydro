...
namespace hydro
{
    ...
    {
        _len = value->_len;
        mBuffer = new char16_t[_len + 1];

        // deep copy
        for (uint32_t i = 0; i < _len; i++)
            mBuffer[i] = value->mBuffer[i];
    ...
    hstring HString::substr(uint32_t pos, int32_t length)
    {
        ...
        if (size - pos <= 0)
            return new HString{mEnv, mClass, ""};
        ...
        buf[size] = '\0';
        return new HString{mEnv, mClass, buf};
    }
