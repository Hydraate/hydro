    ...
    while (!currentFrame->halt && ip < endAddr && ip < bin.size())
    {
        ...
        case objectinit_instr:
        {
            ...
            if (a.type() == typeid(HObject *))
            {
                ...
                obj->mContext->setProperty(_env, this, currentFrame->callee, cxt, name, b);
            }
            else
            {
        ...
        case jsoninit_instr:
        {
            ...
            hjson json = b;         // cast
            json->mContext->setIndex(_env, this, currentFrame->callee, cxt, a, c);
            break;
        }
        case list_instr:
        {
        ...
        case dictinit_instr:
        {
            ...
            hdictionary dict = a;
            dict->mContext->setIndex(_env, this, currentFrame->callee, cxt, b, c);
            break;
        }
        case tup_instr:
        {
        ...
        case callindex_instr: // call index
        {
            ...
            obj = a; // cast
            if (obj->mContext->callIndex(_env, this, currentFrame->callee, cxt, b, args, c))
            {
                if(cxt->errorPending)
                    break;
        ...
        case callpropertyx_instr:
        {
            ...
            if (a.type() == typeid(HObject *))
            {
                ...
                if (obj)
                {
                    if (obj->mContext->callProperty(_env, this, currentFrame->callee, cxt, name, args, b))
                    {
                        if(cxt->errorPending)
                            continue;
        ...
        case chainx_instr:
        {
            ...
            if (a.type() == typeid(HObject *))
            {
                ...
                if (obj)
                {
                    obj->mContext->callProperty(_env, this, currentFrame->callee, cxt, name, args, b);
        ...
        case setpropertyx_instr: // TODO remove
        {
            ...
            {
                obj = a;
                if (obj->mContext->setProperty(_env, this, currentFrame->callee, cxt, name, b))
                    opush(currentFrame, b);
                else
                {
                    // error
        ...
        case getpropertyx_instr: // TODO remove
        {
            ...
            {
                obj = a;
                if (obj->mContext->getProperty(_env, this, currentFrame->callee, cxt, name, b))
                    opush(currentFrame, b);
                else
        ...
        case setindex_instr:
        {
            ...
            obj = a; // cast
            if (obj->mContext->setIndex(_env, this, currentFrame->callee, cxt, b, c))
                opush(currentFrame, c);
            else
                runtimeError(cxt, "Illegal subscript access on " + describe(a));
            break;
        ...
        case getindex_instr:
        {
            ...
            obj = a; // cast
            if (obj->mContext->getIndex(_env, this, currentFrame->callee, cxt, b, c))
                opush(currentFrame, c);
            else
                runtimeError(cxt, "Illegal subscript access on " + describe(a));
            break;
...
void VM::call(HvmContext *threadContext, hvalue callable, std::list<hvalue> &args, hvalue &result, hvalue thisObject)
{
	if (is_callable(callable))
	{
		...
		else
        {
			...
			runtimeError(threadContext, "Attempted invocation of non-callable object '" + get_qualified_name(obj->mContext->classOf()->assemblyName()) + "'.");
        }
	}
...
bool VM::getProperty(HvmContext *cxt, hobject instance, std::string name, hvalue &value)
{
    ...
    return instance->mContext->getProperty(_env, _env->_vm, nullptr, cxt, name, value);
}
...
bool VM::setProperty(HvmContext *cxt, hobject instance, std::string name, hvalue &value)
{
    ...
    return instance->mContext->setProperty(_env, _env->_vm, nullptr, cxt, name, value);
}
...
bool VM::getIndex(HvmContext *cxt, hobject instance, hvalue key, hvalue &value)
{
    ...
    return instance->mContext->getProperty(_env, _env->_vm, nullptr, cxt, key, value);
}
...
bool VM::setIndex(HvmContext *cxt, hobject instance, hvalue key, hvalue &value)
{
    ...
    return instance->mContext->setProperty(_env, _env->_vm, nullptr, cxt, key, value);
}
...
bool VM::callProperty(HvmContext *cxt, std::string name, hobject instance, std::list<hvalue> &args, hvalue &result)
{
    ...
    return instance->mContext->callProperty(_env, this, nullptr, cxt, name, args, result);
}
