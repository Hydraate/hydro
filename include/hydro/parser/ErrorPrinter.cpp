...
ErrorPrinter::ErrorPrinter(Compiler *compiler) : mCompiler{compiler}, _errorsPrinted{0}, printLimit{5}, errorTemplate{"CompilationError in {source} at line {line}: {text}"}, contentPadding{0}, lineNumberPadding{0}, errorPointer{">"}, errorFill{" "}, lineDivider{" |"} {}

ErrorPrinter::~ErrorPrinter() {}

void ErrorPrinter::print()
{
	int32_t nerrors = (int32_t)mCompiler->numErrors();
...
{
	// check error bounds
	if (index < 0 || index >= mCompiler->numErrors())
	...
	// init error data
	std::string errorSrc = mCompiler->errorSourceAt(index);
	std::string errorText = mCompiler->errorTextAt(index);
	int32_t errorLine = mCompiler->errorLineAt(index);
	//uint32_t errorPos = mCompiler->errorIndexAt(index);
