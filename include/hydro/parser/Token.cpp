...
Token::Token(lex_source source, std::string value, int32_t type, uint32_t lineNum, int32_t pos) : _src{source->location()}, _value{value}, _type{type}, mLineNum{lineNum}, mPos{pos}, _prevToken{nullptr}, _nextToken{nullptr} {}

Token::Token(std::string value, int32_t type, uint32_t lineNum, int32_t pos) : _src{}, _value{value}, _type{type}, mLineNum{lineNum}, mPos{-1}, _prevToken{nullptr}, _nextToken{nullptr} {}
...
uint32_t Token::line() const { return mLineNum; }

int32_t Token::position() const { return mPos; }
