...
HydroCodeGenerator::HydroCodeGenerator(HydroCompiler *compiler, Scope *globals, Runtime *runtime) : HydroVisitor{compiler, globals}, _mode{h3o_compile_aot}, _nlabels{0}, _h3oasm{""}, _tab{0}, _runtime{runtime}, _program{nullptr}, mLine{0}, _source{""} {}
...
void HydroCodeGenerator::generate(Program *program, Scope *scopeToUse, hydro_compile_mode mode)
{
	...
	if (!getCompiler()->failed() && evalMode())
	{
	...
	if (static_cast<HydroCompiler *>(mCompiler)->debug())
