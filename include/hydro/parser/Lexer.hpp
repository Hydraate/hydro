...
#ifndef __h3omLexer__
#define __h3omLexer__
...
const std::regex lexer_whitespace_withmLinefeed{"^[\\s\\t\\r\\n]+$"};
...
protected:
    ...
    std::string bufferWithCurrent() { return _currentChar ? mBuffer + _currentChar : ""; }
...
public:
	...
	lex_token currentToken() const { return mCurrentToken; }
	void currentToken(lex_token token) { mCurrentToken = token; }
	lex_source source() const { return _source; }
    int32_t position() const { return mPos; }
    uint32_t line() const { return mLine; }
};

typedef Lexer *lexmLexer;
