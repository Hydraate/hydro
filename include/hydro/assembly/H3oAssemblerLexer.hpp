...
#ifndef __h3o_H3oAssemblerLexer__
#define __h3o_H3oAssemblerLexer__
...
class H3oAssemblerLexer : public Lexer
{
private:
	bool predictHex()
	{
	...
	lex_token string()
	{
		...
		if (!terminated)
			mCompiler->appendError("Unterminated string literal.", source()->location(), line(), position());
...
public:
	H3oAssemblerLexer(HydroCompiler *compiler) : Lexer{compiler}
	{
		registerTokenPattern(id_tkn, lexer_identifier);
	}
	virtual ~H3oAssemblerLexer() {}
};

} // namespace hydro

#endif /* __h3o_H3oAssemblerLexer__ */
