...
HydroParser::HydroParser(HydroCompiler *compiler, Scope *globals) : Parser{new HydroLexer{compiler}}
{
    mAST = new AST{};
    mGlobalScope = globals;
...
void HydroParser::parseFromCurrentToken()
{
    skip(ln_tkn);
...
{
    Parser::reset();
    m_compilationUnit = mAST->createCompilationUnit();
}
...
ast_expr HydroParser::computeExpr(int minPrec)
{
    ...
    )
    {
        ...
        {
            reportError(HVM_COMPILE_ERROR_INVALID_BINARY, a);
            abort();
        }
        ...
        while (true)
        {
            ...
            if (isPostfix(a) && postfixPrec(a) > binaryPrec(b))
            {
                ...
                if (match("++", a) || match("--", a))
                {
                    ...
                    lhs = operands.top();
                    operands.pop();
                    PostfixExpression *postfix = mAST->createNode(a);
            ...
            { // If a > b
                if (match("(", a))
                {
                    ...
                    Invocation *call = mAST->createNode(a);
                    operands.push(call);
                    break;
                }
                else if (match("[", a))
...
Expression *HydroParser::emptyExpr()
{
    return mAST->createEmptyExpression();
}
...
StringExpression *HydroParser::buildString(std::vector<lex_token> &stringTokens)
{
    token_t tkn;
    ...
    if (!stringTokens.empty())
    {
        ...
        // merge tokens
        for (token_t strTkn : stringTokens)
            str += strTkn->value();
    ...
    stringTokens.clear();
    return mAST->createStringExpression(tkn);
}

StringLiteral *HydroParser::parseString()
{
    ...
    std::vector<token_t> stringTokens;

    while (getLexer()->hasNext())
    {
        ...
        if (match("\\"))
        {
            ...
            if (!tokenExists())
            {
                ...
                reportError(HVM_COMPILE_ERROR_INCOMPLETE_ESCAPE_SEQUENCE, currentToken());
                abort();
            }
            ...
            case 'u':
            {
                ...
                // consume u
                stringTokens.push_back(currentToken());
                reportError("Unicode is not current supported.", currentToken());
                abort();
                break;
            }
            case 'U':
            ...
            default:
            {
                ...
                reportError(HVM_COMPILE_ERROR_UNKNOWN_ESCAPE_SEQUENCE, currentToken());
                abort();
            }
            }
        }
    ...
    if (!terminated)
        reportError(HVM_COMPILE_ERROR_UNTERMINATED_STRING_TEMPLATE, tpl->token());
...
{
    validate(id_tkn);
    return mAST->createSimpleName(currentToken());
}
...
Name *HydroParser::parseName()
{
    ...
    if(matchNext(".", ln_tkn))
    {
        ...
        while(matchNext(".", ln_tkn))
        {
            ...
            rhs = parseSimpleName();
            QualifiedName *qname = mAST->createQualifiedName(lhs, rhs)
            result = qname;
        }
    }
    else
...
Identifier *HydroParser::parseID()
{
    validate(id_tkn);
    return mAST->createIdentifier{currentToken()};
}
...
ast_unary HydroParser::parseUnaryOp()
{
    ...
    mCompiler->reportError(HVM_COMPILE_ERROR_INVALID_UNARY, tkn);
    abort();
    return nullptr;
}
...
ast_binary HydroParser::parseBinaryOp(ast_expr lhs, lex_token token, ast_expr rhs)
{
    ...
    reportError(HVM_COMPILE_ERROR_INVALID_BINARY, token);
    abort();
    return nullptr;
}
...
ast_try HydroParser::parseTry()
{
    ...
    else
    {
        // error
        advanceSkip(ln_tkn);
        reportError("Expecting a catch clause after try block.");
    }
...
ast_construct HydroParser::parseConstructor()
{
    ...
    {
        // illegal!
        reportError("Illegal constructor declaration outside of class context.", tkn);
    }
...
ast_property HydroParser::parseProperty()
{
    ...
    {
        // error
        reportError("Property declaration outside of class context.", tkn);
    }
    ...
    else if (matchNext("{", ln_tkn))
    {
        ...
        while (nextTokenExists())
        {
            ...
            if (specGetter())
            {
                ...
                {
                    // error
                    reportError("Redefinition of getter on property '" + name->value() + "'.", tkn);
                }
            ...
            else if (specSetter())
            {
                ...
                {
                    // error
                    reportError("Redefinition of setter on property '" + name->value() + "'.", tkn);
                }
